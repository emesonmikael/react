...
const Home= () => {
  ...
  useEffect(() => {
    const loadM3UFile = async () => {
      try {
        const response = await fetch('/categorias filmes.txt'); // Substitua 'playlist.m3u' pelo nome do seu arquivo
  ...
  const isSupportedMedia = (url) => {
    const supportedFormats = ['mp4', 'webm', 'ogg', 'm3u8', 'mp3', 'wav', 'flac','html'];
  ...
  const handleChannelSelect = (channel) => {
    if (isSupportedMedia(channel.url)) {
      ...
      navigate(`/${encodeURIComponent(channel.name)}?url=${encodeURIComponent(channel.url)}`);
    } else {
